...
from apps.projects.models import Project, ProjectAttachment, ProjectAudit
...
class ProjectModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.valid_data = {
            ...
            "description": (
                "This project develops an AI-based tracker integrated "
                "with IoT devices."
            ),
            "status": "fundraising",
    ...
    def test_slug_auto_generated_from_title(self):
        project = Project.objects.create(
            startup=self.startup, title="My project", target_amount=Decimal("1000.00")
        )
        self.assertEqual(project.slug, "my-project")

    def test_slug_auto_generated_handles_duplicates(self):
        project1 = Project.objects.create(
            startup=self.startup, title="Test project", target_amount=Decimal("1000.00")
        )
        project2 = Project.objects.create(
            startup=self.startup, title="Test project", target_amount=Decimal("2000.00")
        )
...
class ProjectAttachmentModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.startup = StartupProfile.objects.create(
            user=self.user, company_name="Test Startup", email="test@test.com"
        )
    ...
    def test_create_attachment(self):
        SimpleUploadedFile("test.jpg", b"filecontent", content_type="image/jpeg")
        attachment = ProjectAttachment.objects.create(
            project=self.project, type="image", caption="Test image", order=1
        )
...
class ProjectAuditModelTest(TestCase):
    ...
    def test_create_audit_log(self):
        audit = ProjectAudit.objects.create(
            ...
            user=self.user,
            action="update",
            changes={"status": {"old": "idea", "new": "fundraising"}},
        )
    ...
    def test_audit_jsonfield_stores_changes(self):
        ...
        audit = ProjectAudit.objects.create(
            project=self.project, user=self.user, action="update", changes=changes
        )
