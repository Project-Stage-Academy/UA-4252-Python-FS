...
class ProjectModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.valid_data = {
            ...
            "description": "The project aims to develop an AI-based tracker integrated with IoT devices.",
            "status": "In Progress",
    ...
    def test_create_valid_project(self):
        """Ensure a valid Project can be created"""
    ...
    def test_invalid_currency_code(self):
        ...
        invalid_data = self.valid_data.copy()
        invalid_data["currency"] = "US"  # only 2 chars
        project = Project(**invalid_data)
        with self.assertRaises(ValidationError):
            project.full_clean()

    def test_target_and_raised_amount_validation(self):
        """Raised amount should not exceed target amount (custom logic placeholder)"""
        project = Project.objects.create(**self.valid_data)
        project.raised_amount = 100000  # more than target
        self.assertTrue(project.raised_amount > project.target_amount)

    def test_str_method_returns_title(self):
        """__str__ method should return project title"""
        project = Project.objects.create(**self.valid_data)
    ...
    def test_tags_field_allows_long_text(self):
        ...
        long_tags = ",".join(["tag" + str(i) for i in range(50)])
        data = self.valid_data.copy()
        data["tags"] = long_tags
        project = Project.objects.create(**data)
        self.assertIn("tag49", project.tags)
